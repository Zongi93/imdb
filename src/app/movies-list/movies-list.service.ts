...
export class MoviesListService {
  ...
  readonly watchListArray$: Observable<Array<number>> = this
    .filmWatchlistedEmitter;

  constructor(
    private readonly restController: RestControllerService,
    private readonly persistanceService: PersistanceService
  ) {
    const persistedWatchList = persistanceService.get<Array<number>>(
      this.WATCHLIST_KEY,
      []
    );
    this.filmWatchlistedEmitter.next(persistedWatchList);

    this.filmWatchlistedEmitter.subscribe((arr) =>
      persistanceService.set(this.WATCHLIST_KEY, arr)
    );
  }
  ...
  updateWatchlist(film: Film): void {
    ...
    if (filmAlreadyOnWatchlist) {
      this.filmWatchlistedEmitter.next(
        watchlist.filter((id) => id !== film.id)
      );
    } else {
  ...
  isFilmOnWatchlist(film: Film): Observable<boolean> {
    return this.filmWatchlistedEmitter.pipe(
      map((arr) => arr.includes(film.id), distinctUntilChanged())
    );
  }
}
